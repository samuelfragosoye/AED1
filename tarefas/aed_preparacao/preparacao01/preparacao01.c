...
int contar_simbolos_recursivo(const char* texto){
    if (texto[0] == '\0') {    
        return 0;
    } else {
        char caractere_atual = texto[0];
        int contador = 0;
        if (is_maiuscula(caractere_atual)||
           is_minuscula(caractere_atual)||
           is_digito(caractere_atual)   ||
           ...
           return contador + contar_simbolos_recursivo(texto + 1);
    }
...
void method_05(){
    ...
    printf("\nMethod 05 - Contar Simbolos Alfanumericos (Recursivo)\n");
    IO_flush();
    printf("Digite uma sequência de caracteres: ");
    fgets(texto, 80, stdin);
    texto[strcspn(texto, "\n")] = '\0';

    total_simbolos = contar_simbolos_recursivo(texto);
...
printf ( "\n%s" , "5 - Method_05 - Contar Simbolos Alfanumericos (Recursivo)" );
printf ( "\n" );
